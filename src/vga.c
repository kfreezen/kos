...
#define peekb(O) *(unsigned char*)(O)

#define _vmemwr(D, S, C) memcpy((void*)(D), (void*)(S), (C))
...
{
	_vmemwr(get_fb_loc()+dst_off, src, count);
}
/*****************************************************************************
*****************************************************************************/
...
/* tell the BIOS what we've done, so BIOS text output works OK */
	pokew(0x44A, cols);	/* columns on screen */
	pokew(0x44C, cols * rows * 2); /* framebuffer size */
	pokew(0x450, 0);		/* cursor pos'n */
	pokeb(0x460, ht - 1);	/* cursor shape */
	pokeb(0x461, ht - 2);
	pokeb(0x484, rows - 1);	/* rows on screen - 1 */
	pokeb(0x485, ht);		/* char height */
...
{
	kprintf("Screen-clear in 16-color mode will be VERY SLOW\n"
